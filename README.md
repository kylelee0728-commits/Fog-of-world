...
一個步行探索 App：整個世界是暗的，你走過的地方會被永久點亮，
留下屬於自己的世界地圖。走得越多，稱號越高，解鎖成就、蓋滿世界護照。
...
| 動作 | 說明 |
| --- | --- |
| 🚶 **開始探索** | 開啟 GPS，走路時光會沿著路線亮起來，永久保留 |
| 🏅 **成就** | 32 個成就：距離、面積、連續天數、時段、海拔、地標 |
| 🛂 **世界護照** | 123 個世界地標，走進 1 公里內自動蓋章，依大洲分類 |
| 🎯 **定位** | 回到自己的位置 |
| 📊 **統計** | 點頂部數據列開啟：稱號進度、近 14 天長條圖、各項總計 |
| ⚙️ **設定** | 點亮半徑、夜色濃度、GPS 精度門檻、自動更新、匯出匯入 |

**稱號**依點亮的面積晉升：提燈人 → 尋光者 → 拾光者 → 巡路人 → 繪光師 → 破夜者 → 曦光使者 → 白晝之主。
...
最新版在 Releases 頁面，手機直接點 APK 下載開啟即可安裝
（第一次要允許「安裝未知來源的應用程式」）：

**<https://github.com/kylelee0728-commits/Fog-of-world/releases/latest>**

裝好之後就不用再手動來這裡了 —— App 內建自動更新，會在開啟時檢查新版並直接下載安裝。

APK 以固定金鑰簽章（存放在 GitHub Secrets），因此版本之間可以直接覆蓋更新、資料保留。

### 版本規則

**年份尾數.月份[.小版本]** —— 例如 `26.8`（2026 年 8 月）、`26.8.1`（同月的小更新）、`26.9`（9 月）。

`versionName` 是給人看的，Android 判斷新舊看的是 `versionCode`，必須是遞增整數。
所以 `versionCode` 由版本字串自動推導成 **YYMMPP**，不需要手動維護：
...
> 這樣編是為了讓小版本插得進去。若直接用 `2608`／`2609`，中間沒有整數可以給 `26.8.1`，
> 裝置會因為 versionCode 沒有變大而拒絕更新。每個月最多 99 個小版本。

格式錯誤（`26.13`、`26`、`1.8`）會在 Gradle 設定階段直接失敗，CI 也會再擋一次。

### 發佈新版

到 Actions → **Android Release** → Run workflow，填版本號（例如 `26.8.1`）即可。
流程會自動更新 `build.gradle.kts` 的版本、提交、建置、驗證 APK 確實有簽章，
最後發佈成 `v26.8.1` 的 Release。不需要在本機改任何檔案。

也可以推 `v*` 標籤觸發，但此時標籤必須與檔案裡的版本一致，否則會失敗。
...
| 通知 | 前景服務的常駐通知，顯示里程與稱號，可直接暫停 |
| 安裝應用程式 | 自動更新時交給系統安裝程式（可在設定關閉自動更新） |
...
足跡存檔的**格式也是共用的**：兩邊的「匯出存檔」產生的 JSON 完全相同
（同一套 20 公尺網格與量化方式），所以網頁版走出來的圖可以匯進 Android 版，反之亦然。
不過兩邊各自儲存，不會自動同步，要靠匯出／匯入手動搬。

## 運作方式

```
index.html ─ 介面骨架（狀態列 / 底部工具列 / 面板 / 開場）
styles/main.css ─ 長夜風格
vendor/leaflet/ ─ Leaflet 1.9.4（放在本地，離線也能開）
src/
├── main.js        地圖、事件接線、設定
├── fogLayer.js    暗色圖層（Canvas 合成）
├── state.js       足跡網格、里程、成就與地標判定
├── tracker.js     GPS watchPosition／模擬行走
├── storage.js     localStorage 存檔與設定
├── landmarks.js   123 個世界地標
├── achievements.js 32 個成就與稱號
├── ui.js          面板與提示訊息繪製
└── util.js        地理與網格運算
...
3. 繪製時先在遮罩畫布上用「加成」模式畫出所有已探索的圓，再用 `destination-out`
   **一次挖穿**暗色圖層 —— 重疊的地方不會出現接縫，邊緣是柔的。
4. 已點亮面積用另一組 **50 公尺粗網格**統計，比「圓面積相加」準確，因為重疊只算一次。
